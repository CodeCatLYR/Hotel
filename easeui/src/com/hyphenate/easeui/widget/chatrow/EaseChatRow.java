...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        String avatar = message.getStringAttribute(EaseConstant.AVATAR, "");
        showUserAvatar(avatar, userAvatarView);

        if(message.direct() == Direct.RECEIVE){
            EaseUserUtils.setUserNick(message.getStringAttribute(EaseConstant.NICK_NAME, message.getFrom()), usernickView);
        }
    ...
    private void showUserAvatar(String avatar, ImageView imageView){
        if(!TextUtils.isEmpty(avatar)){
            if(!avatar.startsWith("http")){
                avatar = "http://m.xincailiao.com" + avatar;
            }
            ImageLoader.getInstance().displayImage(avatar, imageView);
//            Glide.with(context).load(avatar).into(imageView);
        }else{
            ImageLoader.getInstance().displayImage("drawable://" + R.drawable.ease_default_avatar, imageView);
