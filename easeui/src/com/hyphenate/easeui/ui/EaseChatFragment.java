...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    private final int REQUEST_PERMISSION_CAMERA = 67;
    ...
    protected void initView() {
        ...
        inputMenu.setChatInputMenuListener(new ChatInputMenuListener() {
            ...
            @Override
            public void onVoiceKeyBoardMode(boolean voiceMode) {
                if(voiceMode){
                    AndPermission.with(EaseChatFragment.this)
                            .requestCode(REQUEST_PERMISSION_AUDIO)
    ...
    @PermissionYes(REQUEST_PERMISSION_AUDIO)
    private void getAudioYes(List<String> grantedPermissions) {
        ...
        try {
            ...
            String easeMobKey = appInfo.metaData.getString("EASEMOB_APPKEY");
            String user = EMClient.getInstance().getCurrentUser();

            if(!TextUtils.isEmpty(easeMobKey) && !TextUtils.isEmpty(user)){
                PathUtil.getInstance().initDirs(easeMobKey, user, getActivity());
            }
    ...
    @PermissionNo(REQUEST_PERMISSION_AUDIO)
    private void getAudioNo(List<String> deniedPermissions) {
        // 用户否勾选了不再提示并且拒绝了权限，那么提示用户到设置中授权。
        if (AndPermission.hasAlwaysDeniedPermission(this, deniedPermissions)) {
            // 第一种：用默认的提示语。
            AndPermission.defaultSettingDialog(this, 22).show();
        }else{
            showToast("录音失败，发送语音需要 '录音' 权限");
        }
    }
                ...
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if(getActivity().isFinishing() || !toChatUsername.equals(value.getId()))
                            return;
                        mMemberCount = value.getMemberCount();
                        addChatRoomChangeListenr();
                        onConversationInit();
                        onMessageListInit();

                        if(mChatRoomMemberListener != null){
                            mChatRoomMemberListener.onMemberCountChange(mMemberCount);
                        }
                    }
                });
            }
    ...
    @PermissionYes(REQUEST_PERMISSION_LOCATION)
    private void getLocationYes(List<String> grantedPermissions) {
    }

    @PermissionNo(REQUEST_PERMISSION_LOCATION)
    ...
    protected void sendMessage(EMMessage message){
        ...
        EaseUser user = EaseUI.getInstance().getCurrentUser();
        if(user != null){
            message.setAttribute(EaseConstant.AVATAR, user.getAvatar());
            message.setAttribute(EaseConstant.NICK_NAME, user.getNickname());
        }
...
//            });
    ...
    public void setChatRoomMemberListener(ChatRoomMemberListener mChatRoomMemberListener) {
        this.mChatRoomMemberListener = mChatRoomMemberListener;
    }


    public interface  ChatRoomMemberListener{
        public void onMemberCountChange(int memberCount);
    }
